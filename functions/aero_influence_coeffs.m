function a_coeffs = aero_influence_coeffs( M,geom_vec,aero_vec,b,sweep )
...
for n = n_idxs
    % Eq. (A37) case n =/= nu
    B       = dwing.littlebfun( nu,n ) + dwing.littlebfun( nu,m+1-n );
    a_nu(n) = -2*B + b/c*dwing.gbarfun( nu,n );
end
...
B          = dwing.littlebfun( nu,m_end );
a_nu(m_end)= -2*B + dwing.b/dwing.geom_sect(m_end).c*dwing.gbarfun( nu,m_end );
% Adding nu point
a_nu(nu)   = 2*(dwing.m+1)/( 4*sin(dwing.geom_sect(nu).phi) );    %b_nu,nu expression
if nu == m_red
    % case nu = n = (m+1)/2
     a_nu(nu) = a_nu(nu) + dwing.gbarfun_special( nu,nu );
else
    % case nu = n =/= (m+1)/2
    a_nu(nu) = a_nu(nu) + dwing.gbarfun( nu,nu );
end
