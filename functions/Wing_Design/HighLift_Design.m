...
while QT > 1
    ...
    if deltaSs == 0
        wing3Ddata(m)          = aero_des.low_speed.aero3Dwing( 'Take-off',aero_des.low_speed.prf3Dclean.M.root.M,deltaFs,deltaSs );
    else
        wing3Ddata(m)          = aero_des.low_speed.aero3Dwing( 'Landing',aero_des.low_speed.prf3Dclean.M.root.M,deltaFs,deltaSs );
    end
    
    %% Results Display
    ax_HL(1) = subplot(1,2,1,'Parent',fig_HL);
    ...
    CL_clean = aero_des.low_speed.aero3Dwing.lift_eval( alpha,aero_des.low_speed.prf3Dclean );
    CL_hl    = aero_des.low_speed.aero3Dwing.lift_eval( alpha,wing3Ddata(m) );
